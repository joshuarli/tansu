...
describe("getCursorMarkdownOffset", () => {
  ...
  beforeAll(async () => {
    ...
    getCursorMarkdownOffset = serMod.getCursorMarkdownOffset;
  });
  ...
  test("start of plain paragraph → offset 0", () => {
  ...
  test("middle of plain paragraph", () => {
    const el = makeEl("<p>hello</p>");
    const text = el.querySelector("p")!.firstChild as Text;
    expect(getCursorMarkdownOffset(el, rangeAt(text, 3))).toBe(3);
  });

  test("end of plain paragraph", () => {
    const el = makeEl("<p>hello</p>");
    const text = el.querySelector("p")!.firstChild as Text;
  ...
  test("cursor inside bold — before first char → offset 2 (after **)", () => {
  ...
  test("cursor inside bold — after first char → offset 3", () => {
    const el = makeEl("<p><strong>bold</strong></p>");
    const text = el.querySelector("strong")!.firstChild as Text;
    expect(getCursorMarkdownOffset(el, rangeAt(text, 1))).toBe(3);
  });

  test("cursor inside bold — after last char → offset 6", () => {
    const el = makeEl("<p><strong>bold</strong></p>");
    const text = el.querySelector("strong")!.firstChild as Text;
    expect(getCursorMarkdownOffset(el, rangeAt(text, 4))).toBe(6);
  });

  test("cursor after strong element → offset 8 (**bold**)", () => {
    const el = makeEl("<p><strong>bold</strong></p>");
  ...
  test("cursor inside italic — before first char → offset 1 (after *)", () => {
  ...
  test("cursor inside italic — after last char → offset 3 (*hi)", () => {
    const el = makeEl("<p><em>hi</em></p>");
    const text = el.querySelector("em")!.firstChild as Text;
  ...
  test("cursor in prefix text before bold", () => {
  ...
  test("cursor inside bold after prefix text", () => {
  ...
  test("cursor at start of second paragraph → offset = first.length + 1", () => {
  ...
  test("cursor in middle of second paragraph", () => {
  ...
  test("cursor inside strikethrough — before first char → offset 2 (after ~~)", () => {
    const el = makeEl("<p><del>bye</del></p>");
    const text = el.querySelector("del")!.firstChild as Text;
    expect(getCursorMarkdownOffset(el, rangeAt(text, 0))).toBe(2);
  });

  test("cursor inside highlight — before first char → offset 2 (after ==)", () => {
