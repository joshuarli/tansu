...
describe("offline resilience", () => {
  ...
  it("openTab caches note content to IDB", async () => {
    cleanState();
  ...
  it("openTab falls back to IDB cache when server is down", async () => {
    cleanState();
  ...
  it("openTab throws when server is down and no cache exists", async () => {
    cleanState();
    mock.on("GET", "/api/note", "server error", 500);
    ...
    try {
      ...
      threw = true;
      expect((error as Error).message).toContain("not available offline");
    }
  ...
  it("switchTab falls back to IDB cache when server is down", async () => {
    cleanState();
  ...
  it("markClean caches updated content to IDB", async () => {
    cleanState();
  ...
  it("closeTab caches note content to IDB", async () => {
    cleanState();
  ...
  it("restoreSession falls back to IDB when server is down", async () => {
    cleanState();
  ...
  it("syncToServer pushes cached state to server", async () => {
    cleanState();
  ...
  it("restoreSession restores closed-tab stack even with no open tabs", async () => {
    cleanState();
...
describe("closed-tab stack", () => {
  ...
  it("close pushes to stack, reopen pops in LIFO order", async () => {
    cleanState();
  ...
  it("reopenClosedTab handles unavailable note gracefully", async () => {
    cleanState();
  ...
  it("reopenClosedTab does nothing when stack is empty", async () => {
    cleanState();
  ...
  it("reopening already-open tab switches to it instead of duplicating", async () => {
    cleanState();
  ...
  it("closed stack is bounded to 20", async () => {
    cleanState();
    ...
    expect(getTabs()).toHaveLength(25);
    ...
    const paths = getTabs().map((t) => (t as { path: string }).path);
  ...
  it("restoreSession restores closed-tab stack", async () => {
    cleanState();
