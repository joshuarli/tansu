...
function parseBlocks(lines: string[]): Block[] {
  ...
  while (i < lines.length) {
    ...
    if (line.trimStart().startsWith('|') && i + 1 < lines.length && /^\|?[\s:]*-+[\s:]*(\|[\s:]*-+[\s:]*)*\|?\s*$/.test(lines[i + 1]!)) {
    ...
    while (i < lines.length) {
      ...
      if (l.trimStart().startsWith('|') && i + 1 < lines.length && /^\|?[\s:]*-+/.test(lines[i + 1] ?? '')) break;
      paraLines.push(l);
      i++;
    }
...
function renderBlockquote(bqLines: string[]): string {
  ...
  if (calloutMatch) {
    ...
    return `<div class="callout callout-${escapeHtml(type)}" data-callout="${escapeHtml(type)}">` +
