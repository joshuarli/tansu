...
interface FieldScores { title: number; headings: number; tags: number; content: number }
interface Result { path: string; title: string; excerpt: string; score: number; field_scores: FieldScores }
...
for (const r of results) {
  ...
  console.log(`\x1b[1m${r.title}\x1b[0m  \x1b[2m${r.path}\x1b[0m  score=${r.score.toPrecision(3)}${breakdown}`);
