...
function applyHighlightToSelection(): boolean {
  ...
  // When the selection crosses block boundaries, cloneContents includes block elements
  // (<p>, <h1>, etc.). Wrapping them all in a single <mark> is invalid HTML since <mark>
  // is inline. Instead, apply <mark> inside each block separately.
  const isMultiBlock = range.commonAncestorContainer === contentEl;

  if (isMultiBlock) {
    const div = document.createElement("div");
    div.appendChild(range.cloneContents());
    const html = buildPerBlockMarkHtml(div, "mark");
    if (
    ...
    applyMarkPerBlock(contentEl, range);
    return true;
  }
...
const HIGHLIGHTABLE_BLOCK_TAGS = new Set(["P", "DIV", "H1", "H2", "H3", "H4", "H5", "H6"]);

function buildPerBlockMarkHtml(div: HTMLDivElement, tag: string): string {
  let html = "";
  for (const child of div.childNodes) {
    if (child instanceof HTMLElement && HIGHLIGHTABLE_BLOCK_TAGS.has(child.tagName)) {
      if (isEffectivelyBlank(child)) {
        ...
        const attrs = Array.from(child.attributes)
          ...
          .join("");
        html += `<${t}${attrs}><${tag}>${child.innerHTML}</${tag}></${t}>`;
      }
    } else {
      const span = document.createElement("span");
      span.appendChild(child.cloneNode(true));
      html += `<${tag}>${span.innerHTML}</${tag}>`;
    }
  }
  return html;
}

function isEffectivelyBlank(el: HTMLElement): boolean {
  return (el.textContent ?? "").replace(/\u200B/g, "").trim() === "";
}

function applyMarkPerBlock(contentEl: HTMLElement, range: Range): void {
  const blocks = Array.from(contentEl.children).filter(
    (el) => el instanceof HTMLElement && range.intersectsNode(el),
  ) as HTMLElement[];

  const blockRanges: Range[] = [];
  for (const block of blocks) {
    if (isEffectivelyBlank(block)) continue;

    const blockRange = document.createRange();
    const startInBlock = block.contains(range.startContainer);
    const endInBlock = block.contains(range.endContainer);
  ...
  for (const blockRange of blockRanges) {
    const mark = document.createElement("mark");
    mark.appendChild(blockRange.extractContents());
